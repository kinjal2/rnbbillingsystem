...
//dd($oldBilldetail);
        $data = [
			'cust_no' => $oldBilldetail[0]->cust_no,
            'cust_name' => $oldBilldetail[0]->cust_name,
			'sector_no'=> $oldBilldetail[0]->sector_no,
            'plot_no'=> $oldBilldetail[0]->plot_no,
            'cheque_no'=> $oldBilldetail[0]->cheque_no,
            'created_at'=> $oldBilldetail[0]->created_at,
            'bank_name'=>$oldBilldetail[0]->bank_name,
            'branch_name'=> $oldBilldetail[0]->branch_name,
            'amount'=> $oldBilldetail[0]->amount,
            'amountword'=> Terbilang::make($oldBilldetail[0]->amount," Only" , 'Rupees '),
        ...
        foreach ($data as $chunk) {
            $html = view('pdf.invoice', ['data' => $chunk])->render();
            $mpdf->WriteHTML($html);
            // Add a page break after each chunk
            $mpdf->AddPage();
        }
        
        $mpdf->Output('large_invoice.pdf', 'D'); // 'D' for download, 'I' for inline display,
